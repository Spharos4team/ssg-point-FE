...
const ClubContentBeauty = () => {
  return (
    <>
      ...
      <ToggleCard title="뷰티 클럽 가입" toggleId="toggle08" modified="" detail={false} />
      <NoticeBox className="!p-5" type="strong" title="해당 브랜드">
        {Object.entries(brandList).map((value) => (
          <ListForNoticeBox key={value[0]} className="inline-block w-1/3" level={1}>
            {value[1]}
          </ListForNoticeBox>
        ))}
      </NoticeBox>
      ...
      <ListForNoticeBox className="inline-block mt-4 mb-6 mx-5" level={2}>
        이마트 매장에서 브랜드 화장품 구매 시 신세계포인트 2~5% 추가 적립 혜택을 받으실 수 있습니다.
      </ListForNoticeBox>

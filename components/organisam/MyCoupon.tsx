...
import { dateFormatter, getDDay, dateHyphenSlashConverter } from '@/utils/FormatHelpers';
...
export default function MyCoupon() {
  ...
  const expirationList = couponFetchList.filter((i) => i.pathname == whiteList[1]);
  const getMessage = (currPathname: any) => {
    switch (currPathname) {
      case "/benefits/myCoupon":
        return "사용가능한 쿠폰이 없습니다.";
      case "/benefits/endCoupon":
        return "사용 완료/만료된 쿠폰이 없습니다.";
    }
  };
  ...
  useEffect(() => {
    if (currPathname == whiteList[0]) {
      const thisFetchValue = FetchListByFetchItem(myCouponFetchValue as string, usableList);
      thisFetchValue
        .then((v) => {
          setEventList(v);
        })
      ...
      const thisFetchValue = FetchListByFetchItem(myCouponFetchValue as string, expirationList);
      thisFetchValue
        .then((v) => {
          setEventList(v);
        })
      ...
      <div className={`px-5`}>
        {currPathname === whiteList[0] ? (
          <>
            ...
            <ListHeader border>
              {/* dropdown */}
              <Dropdown id={KEY.MYCOUPONS} options={usableList.map((i) => i.name)} />
            </ListHeader>
          </>
          ...
          <>
            ...
            <ListHeader border>
              {/* dropdown */}
              <Dropdown id={KEY.MYCOUPONS} options={expirationList.map((i) => i.name)} />
            </ListHeader>
          </>
        ) : (
          ''
        ...
        <ListBody>
          {couponList ? (
            couponList.map((coupon: CouponType) => (
              <li key={coupon.id} className="flex justify-between gap-x-2 pt-4 border-b">
                ...
                <div className="flex flex-col justify-between w-4/5 overflow-hidden">
                  <div className="h-20 border-b">
                    ...
                    <p className="pb-3 text-gray-500 text-[11px] whitespace-nowrap overflow-hidden text-ellipsis">
                      {dateFormatter(coupon.start_date)} ~ {dateFormatter(coupon.end_date)}

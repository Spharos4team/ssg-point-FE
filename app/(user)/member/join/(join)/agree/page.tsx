  ...
  return (
    <>
      <div className="px-5 py-10">
        <CertNavigation />
        <ContentTitle className="font-medium">약관동의</ContentTitle>
      </div>
      <div className="px-5">
        <Button backgroundColor={'gray'} className="h-12 mb-3 border-0 font-medium">
          모두 동의합니다.
        </Button>
        <ListForNoticeBox level={2} isReference>
          전체 동의에는 필수 및 선택 정보수집에 대한 동의가 포함되어 있으며, 개별적인 동의 선택도
          가능합니다. 선택항목에 대한 동의를 거부하시더라도 필수 서비스 이용은 가능합니다.
        </ListForNoticeBox>
      </div>
      <div className="pt-10 px-5 ">
        <h3 className="font-medium pb-4 leading-7 text-lg">신세계포인트 통합회원</h3>
            ...
            <li className="flex items-center justify-between">
              ...
              <Modal id={`${term.id}_valid`} center>
                <p className="text-sm tracking-[-.3px]">
                  {term.required ? '[필수]' : '[선택]'} {term.name}에 동의해주세요.
                </p>
                <Button
                  className="mt-5 !h-10"
                  bgColor="black"
      ...
      <NoticeBox type={'info'} title={'유의사항'} className="px-5">
        <ListForNoticeBox level={2} isBullet>
          선택항목 수집 및 이용동의를 거부하시더라도 기본 서비스는 이용하실 수 있습니다.
        </ListForNoticeBox>
        <ListForNoticeBox level={2} isBullet>
          이미 신세계포인트 회원이신 경우 신세계포인트 온라인 통합ID 가입 시 입력한 정보로
          회원정보가 변경됩니다.
        </ListForNoticeBox>
      </NoticeBox>
      {/*<ContentHeader>*/}
